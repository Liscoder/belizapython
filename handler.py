...
def try_send_message(user, message):
	...
	flag = check_last_connection(user) or validate_quick_replies(user, message)
...
def set_user_attachments(user, attachments):
	for attachment in attachments:
		if attachment['type'] == 'location':
			coordinates = attachment['payload']['coodinates']
...
def get_message_data(user, message, data_model = {}):
	...
	elif type_message == 'create_quick_location':
...
def add_user_location(user,lat,lng):
	...
	locations.append({'lat':lat, 'lng':lmg, 'city':data_model['city'],'created_at':datetime.datetime.now()})
